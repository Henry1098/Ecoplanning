...
void __fastcall TplStandard::Load_Previsionnel()
{
 ...
 for (ix=1;ix<=LASTTASK;ix++)  //nb_taches;ix++)
 {
   ...
   //{
     ...
     S =  av->Cells[42][ix];
     AdvStringGrid1->Cells[15][cnt]= av->Cells[42][ix];
     AdvStringGrid1->Cells[16][cnt]= av->Cells[41][ix];
     AdvStringGrid1->Cells[17][cnt]= av->Cells[43][ix];

     AdvStringGrid1->Cells[18][cnt]= av->Cells[15][ix];   // ChL 1
     AdvStringGrid1->Cells[19][cnt]= av->Cells[16][ix];
     AdvStringGrid1->Cells[20][cnt]= av->Cells[17][ix];
     AdvStringGrid1->Cells[21][cnt]= av->Cells[18][ix];   // Cout > 0
     AdvStringGrid1->Cells[22][cnt]= av->Cells[19][ix];   // Cout < 0
 ...
 for (ixx=Lim1;ixx<Lim2;ixx++)
   {
    ...
    if (X2>=0)
       { imgg->Canvas->Pen->Width = 4;
         ...
         else imgg->Canvas->Pen->Color = clBlue;
         ...
         if (reel)
           {
            ...
            if (xrow !=0)
             {
               RetTotal = asAvanc->Cells[7][xrow];
               RetImput = asAvanc->Cells[6][xrow];
               Texte = "";
               if (RetTotal!="0") Texte = Texte + "T"+RetTotal;
               if (RetImput!="0") Texte = Texte + " I" + RetImput;
               imgg->Canvas->Brush->Style = bsClear;
               imgg->Canvas->TextOutA(X2+5,YA-10,Texte);
              }
         ...
         imgg->Canvas->Font->Height = 4;  // pas d'effet
         imgg->Canvas->Brush->Style = bsClear;
      ...
      int ACol, int ARow, AnsiString &Value, bool &Valid)
{
 ...
 case 10 :  // calendrier
  ...
  case 13 : // Fin Souhaitée
    ...
    break;
  ///// ANOMALIE
  //case 14: av->Cells[15][ARow]=Value; Valid=true; break;
  case 18: av->Cells[15][ctask]=Value; Valid=true; break;   // champ libre 1
  case 19: av->Cells[16][ctask]=Value; Valid=true; break;   // champ libre 2
  case 20: av->Cells[17][ctask]=Value; Valid=true; break;   // champ libre 3
  case 21: av->Cells[18][ctask]=Value; Valid=true; break;   // Cout >0
  case 22: av->Cells[19][ctask]=Value; Valid=true; break;   // Cout < 0
   } // end Switch
  ...
  while (rc) //  &strcmp(RetKey,"T0000")!=0)
   {
    ...
    if (ok)
      { count++;
        ...
        Ghost->ExtractValue(tmp,buffer,"dr",0); asAvanc->Cells[4][count] = AnsiString(tmp); av->Cells[23][numtache]=AnsiString(tmp);
        Ghost->ExtractValue(tmp,buffer,"dfr",0); asAvanc->Cells[5][count] = AnsiString(tmp); //setvalue(5,count,43,tmp);
        av->Cells[22][numtache]=AnsiString(tmp);
        Ghost->ExtractValue(tmp,buffer,"rimp",0); asAvanc->Cells[6][count] = AnsiString(tmp); av->Cells[24][numtache]=AnsiString(tmp);
        Ghost->ExtractValue(tmp,buffer,"rtot",0); asAvanc->Cells[7][count] = AnsiString(tmp); av->Cells[25][numtache]=AnsiString(tmp);
        Ghost->ExtractValue(tmp,buffer,"ddp",0); asAvanc->Cells[8][count] = AnsiString(tmp); //setvalue(5,count,46,tmp);
        Ghost->ExtractValue(tmp,buffer,"dp",0); asAvanc->Cells[9][count] = AnsiString(tmp); //setvalue(5,count,47,tmp);
        Ghost->ExtractValue(tmp,buffer,"dfp",0); asAvanc->Cells[10][count] = AnsiString(tmp); //setvalue(5,count,48,tmp);
        ...
        Ghost->ExtractValue(tmp,buffer,"mt",0); asAvanc->Cells[16][count] = AnsiString(tmp); //setvalue(5,count,66,tmp);
        Ghost->ExtractValue(tmp,buffer,"dtposs",0); asAvanc->Cells[17][count] = AnsiString(tmp); //setvalue(5,count,69,tmp);
        Ghost->ExtractValue(tmp,buffer,"prest",0); asAvanc->Cells[18][count] = AnsiString(tmp); //setvalue(5,count,71,tmp);
        Ghost->ExtractValue(tmp,buffer,"jrest",0); asAvanc->Cells[19][count] = AnsiString(tmp); //setvalue(5,count,72,tmp);
        Ghost->ExtractValue(tmp,buffer,"margrl",0); asAvanc->Cells[15][count] = AnsiString(tmp); //setvalue(5,count,73,tmp);
   ...
   if (av->Cells[1][i] != "" && filtr[i]=='X')
     {
       ...
       for (j=1;j<15;j++) AdvStringGrid1->Cells[j][ndx]= av->Cells[j][i];
     // S =  av->Cells[42][i];
     AdvStringGrid1->Cells[15][ndx]= av->Cells[42][i];
     AdvStringGrid1->Cells[16][ndx]= av->Cells[41][i];
     AdvStringGrid1->Cells[17][ndx]= av->Cells[43][i];

     AdvStringGrid1->Cells[18][ndx]= av->Cells[15][i];   // ChL 1
     AdvStringGrid1->Cells[19][ndx]= av->Cells[16][i];
     AdvStringGrid1->Cells[20][ndx]= av->Cells[17][i];
     AdvStringGrid1->Cells[21][ndx]= av->Cells[18][i];   // Cout > 0
     AdvStringGrid1->Cells[22][ndx]= av->Cells[19][i];
       ...
       avc->Cells[8][ndx]=av->Cells[41][i];
       avc->Cells[9][ndx]=av->Cells[42][i];
       avc->Cells[10][ndx]=av->Cells[43][i];
     }
  }
...
{
 int i; char per[100]; char tmp[200]; char dt1[20];
        ...
        if (t==nt)
          {
           ...
           if (asAvanc->Cells[4][t]=="") asAvanc->Cells[4][t]=asAvanc->Cells[9][t];
 ...
 for (it=1;it<=nbtaches;it++)
  {
   ...
   strcpy(tmp,asAvanc->Cells[16][it].c_str());   // marge Totale
   // strcpy(tmp,"0");
   marG=atoi(tmp);
   if (marG==0) im->Canvas->Pen->Color = clRed;
       ...
       if (cal[jcal][dist1+jx] != 'N')
         {
           ...
           R = Rect(x1,ya,x2,y2);
           if (marG==0) im->Canvas->Brush->Color = clRed;
   ...
   im->Canvas->Pen->Color=clWhite;
   im->Canvas->Pen->Width = 10*spy/100;
   if (marG==0) im->Canvas->Pen->Color = clGreen;
       ...
       if (cal[jcal][dist1+jx] != 'N')
         {
           ...
           x2 = x1+ spx; y2=ya+epaisseur; // 10 epaisseur
           R = Rect(x1,ya,x2,y2);
           if (marG==0) im->Canvas->Brush->Color = clGreen;
         ...
         if (strlen(buf)>5)
               {
                ...
                av->Cells[41][nt]=tmp;
                // if (av->Cells[41][nt]=="P-" || av->Cells[41][nt]=="P---"  || av->Cells[41][nt]=="P----") { av->Cells[41][nt]="P----"; 
 ...
 if (fp)
   {
    ...
    for (i=1;i<count;i++)
    {
      if (av->Cells[1][i] != "")
       ...
       if (av->Cells[41][i] == "P-" || av->Cells[41][i] == "P---" || av->Cells[41][i] == "P----" )
          { av->Cells[41][i] = "P---" ; av->Cells[51][i] == "PHASES" ; }
       strcat(tmp,"<c1>"); strcat(tmp,av->Cells[41][i].c_str()); strcat(tmp,"</c1>");
       strcat(tmp,"<d1>"); strcat(tmp,av->Cells[51][i].c_str()); strcat(tmp,"</d1>");


       if (av->Cells[42][i] == "I-" || av->Cells[42][i] == "I---" || av->Cells[42][i] == "I----" )
          { av->Cells[42][i] = "I---" ; av->Cells[52][i] == "INTERVENANTS" ; }
       strcat(tmp,"<c2>"); strcat(tmp,av->Cells[42][i].c_str()); strcat(tmp,"</c2>");
       strcat(tmp,"<d2>"); strcat(tmp,av->Cells[52][i].c_str()); strcat(tmp,"</d2>");

       if (av->Cells[43][i] == "T-" || av->Cells[43][i] == "T---" || av->Cells[43][i] == "T----" )
          { av->Cells[43][i] = "T---" ; av->Cells[53][i] == "TRIS" ; }
       strcat(tmp,"<c3>"); strcat(tmp,av->Cells[43][i].c_str()); strcat(tmp,"</c3>");
       strcat(tmp,"<d3>"); strcat(tmp,av->Cells[53][i].c_str()); strcat(tmp,"</d3>");

       if (av->Cells[44][i] == "R-" || av->Cells[44][i] == "R---" || av->Cells[44][i] == "R----" )
          { av->Cells[44][i] = "R---" ; av->Cells[54][i] == "RESSOURCES" ; }
     ...
     if (P_CJTFLAG[pc] == "Yes")
      {
       ...
       // if (P_CYNJNT[pc]=="Oui") bas = bas*5/7;
       if (P_MAXDIST[pc] != 0)
         { itmp = bas/(P_MAXDIST[pc]-IXDEB);
           if (itmp==0) itmp=1;
           itmp=itmp*1.1;
    ...
    if (nbsc>0 && nbsc<50)
      {
       ...
       y1=atoi(xxx);
       strcpy(tmp,asTri->Cells[60][indx].c_str());
       count=Explode('|',tmp);
       for (loop=1;loop<count;loop++)
        {
          tsucc=atoi(params[loop]);
          ...
          if (rc > 0)
           {
            strcpy(xxx,asTri->Cells[65][rc].c_str());
            x2=atoi(xxx);
    ...
    if (av->Cells[1][i] != "" && av->Cells[1][i] != " ")
      {
        ...
        avc->Cells[8][indx]=av->Cells[41][i];
        avc->Cells[9][indx]=av->Cells[42][i];
        avc->Cells[10][indx]=av->Cells[43][i];
       }
   }
 Calcul_Echeancier();
}
    ...
    if (tach_ech == tach_adv)
       { if (ACol== 6)
        { AdvStringGrid1->Cells[18][i] = avc->Cells[6][ARow];
         ...
         if (ACol== 7)
           { AdvStringGrid1->Cells[19][i] = avc->Cells[7][ARow];
...
for (i=1;i<cnt;i++)
  {
   ...
   if (numt>0 && numt<=LASTTASK)
     { ix++; if (ix>1) avc->RowCount++;
       ...
       avc->Cells[8][ix] = av->Cells[41][i];
       avc->Cells[9][ix] = av->Cells[42][i];
       avc->Cells[10][ix] = av->Cells[43][i];
...
void __fastcall TplStandard::Compute_AV_1()
{
 int nbavanc; char AVANC[5000]; int i;
 char dt1[20]; char dt[20];
 int dist1,a1,m1,j1,a2,m2,j2;
 int dist_avanc_1;  char tmp[250];
 int xx,dist_xx; int distavanc;

 strcpy(AVANC,dateavanc);

 strcpy(dt1,DATEAVANC);
  dt1[2]=0; j1=atoi(dt1);  dt1[5]=0; m1=atoi(dt1+3);  dt1[8]=0; a1=atoi(dt1+6);
  distavanc = convert_date(a1+2000,m1,j1);

  nbavanc=Explode('@',AVANC);
  if (nbavanc==1 || nbavanc==0)
    {  strcpy(adv,asAvanc->Cells[8][i].c_str()); strcpy(dt1,adv);   // debut prev
       strcpy(dt,dt1);
       dt1[2]=0; j1=atoi(dt1);  dt1[5]=0; m1=atoi(dt1+3);  dt1[8]=0; a1=atoi(dt1+6);
       dist_avanc_1 = convert_date(a1+2000,m1,j1);
       AA_AV_1=a1; MM_AV_1=m1; JJ_AV_1=j1;


    }
  else // trouver  date actualisation précedente   par les distances
    {
     // dist_avanc_1 = 0;
     for (i=0;i<nbavanc;i++)    //cntavanc)
       {
        strcpy(dt1,params[i]);
        dt1[2]=0; a1=atoi(dt1);  dt1[5]=0; m1=atoi(dt1+3);  dt1[8]=0; j1=atoi(dt1+6);
        dist_xx = convert_date(a1+2000,m1,j1);
        if (dist_xx==distavanc)
           {
            xx=i-1;
            strcpy(dt1,params[xx]);
            dt1[2]=0; a1=atoi(dt1);  dt1[5]=0; m1=atoi(dt1+3);  dt1[8]=0; j1=atoi(dt1+6);
            dist_avanc_1 = convert_date(a1+2000,m1,j1);
            AA_AV_1=a1; MM_AV_1=m1; JJ_AV_1=j1;
            break;
...
{
  int cntavanc,ctrow, i; int dist_avanc_1;  char tmp[250];
  int nbpr;   char dt1[500]; int dist1,a1,m1,j1,a2,m2,j2;
  int tcon; bool select;  char dt[200];  int distfin;
  char dtfin[200]; bool OK;    int nt; char snumt[20];
  int distavanc;  int percentrestant;   int a;  char AVANC[5000];
  char msg[5000]; char DATEAVANC_REVERSED[1500];
   int nbavanc;
  int dist_xx;  int xx;
  ...
  // sprintf(msg,"TEST SUR CALCUL  nb avanc = %d DATEAVANC %s",nbavanc,DATEAVANC);
  // Application->MessageBox(msg,"Ecopl",MB_OK);
  ...
  for (i=1;i<cntavanc;i++)
    {
      ...
      /// {
         ...
         // if (dist1>=distavanc  || distdebprev >= distavanc)
         if (dist_avanc_1 >= distavanc)
         continue;
         ...
         else  // concern  == true;
           {
             if (!strict)
               ...
               if (IsConcern(nt))   OK = true;
                 ...
                 // if (percentrestant==100) OK=true;
                 if (dist_avanc_1 <= distavanc) { OK=true; } // Application->MessageBox("distprev < distavanc","Ecoplanning",MB_OK); }
               ...
               if (OK)
               ...
               else // strict
                 {
                  ...
                  // if (distdebprev <= distavanc) { OK=true; }
...
void __fastcall TplStandard::Refresh_Colors_Avact()
{
  int cnt,i;
  cnt = avact->RowCount;
  for (i=1;i<cnt;i++)
    {
     if (avact->Cells[3][i]=="X" || avact->Cells[4][i]=="X")  avact->Colors[1][i]=clWhite;
     else avact->Colors[1][i]=clYellow;
...
//  if (ACol==4) valid = Call_Process(29,xcal,"0");
...
{
 int i,nt,cntrow,from; char snumt[50]; char tmp[150]; char str[400];
...
//  Refresh_Concern(true);

 Compute_D1_D2();

  // sprintf(str,"Display FROM=%d D1=%d D2=%d",from,D1,D2);
  // Application->MessageBox(str,"Ecopl",MB_OK);
   ...
   if (DMAX >= 0)
    {
     ...
     // $$$$$$add_jour(AnPrev,MoisPrev,JourPrev,D1);
      ...
      AA_AV_1 = 11; MM_AV_1=1; JJ_AV_1=31;

     add_jour(AA_AV_1,MM_AV_1,JJ_AV_1,D1);
     sprintf(dt2,"%02d/%02d",Jour,Mois);
     AnREF=An; MoisREF=Mois; JourREF = Jour;

      //sprintf(str,"Display DMAX=%d An_1=%d mois_1=%d Jour_1=%d",DMAX,AA_AV_1,MM_AV_1,JJ_AV_1);
      //Application->MessageBox(str,"Ecopl",MB_OK);
     for (i=D1;i<(D1+DMAX+D2+2);i++) // i=i+step)
      {
       ...
       // add_jour(an_1,mois_1,jour_1,i);


       //sprintf(str,"Display DMAX=%d An_1=%s mois_1=%s Jour_1=%s",DMAX,an_1,mois_1,jour_1);
       //Application->MessageBox(str,"Ecopl",MB_OK);

       // $$$$$$add_jour(AnPrev,MoisPrev,JourPrev,i);
       add_jour(AA_AV_1,MM_AV_1,JJ_AV_1,i);
     ...
     for (i=D1;i<(D1+DMAX+D2);i++) // i=i+step)
      {
       ...
       // $$$$$$ add_jour(AnPrev,MoisPrev,JourPrev,i);
        ...
        add_jour(AA_AV_1+2000,MM_AV_1,JJ_AV_1,i);

       // sprintf(str,"Display ANPREV=%d MOISPREV= %d JOURPREV=%d" ,AnPrev,MoisPrev,JourPrev);
      ...
      sprintf (dtPrev,"%02d-%02d-%02d",jour_1,mois_1,an_1%100);
      sprintf (dtAvanc,"%02d-%02d-%02d",JourAvanc,MoisAvanc,AnAvanc%100);
 ...
 void __fastcall TplStandard::Compute_D1_D2()
 {
  ...
  char tmp[200]; char an[20],mois[20],jour[20];  char str[400];
   ...
   { strcpy(FNAVANC,""); D1=0; DIST_AVANC_1=0;
  ...
  else if (CntAVBefore == 1)
    {
      ...
      strcat(FNAVANC,tmp);
           ...
           DIST_AVANC_1 = dist;
           an_1=AnPrev; mois_1=MoisPrev; jour_1=JourPrev;
           //sprintf(str,"CntBefore=1 DIST_AVANC_1 = %d",DIST_AVANC_1);
           //Application->MessageBox(str,"Ecopl",MB_OK);
        ...
        if (strcmp(tmp,DATEAVANC)==0)
          {
           ...
           //sprintf(str,"CntBefore=%d DIST_AVANC_1 = %s",i, DIST_AVANC_1);
           //Application->MessageBox(str,"Ecopl",MB_OK);
       ...
       else
          {
           ...
           dist = Reverse_Date(tmp);
           DIST_AVANC_1 = dist;
           sprintf(str,"Blanc NON TRAITE DIST_AVANC_1 = %d",i, DIST_AVANC_1);
           Application->MessageBox(str,"Ecopl",MB_OK);
           */
...
void __fastcall TplStandard::REVERSE_DATE(char *dt)
{
 ...
 void __fastcall TplStandard::Compute_DD1_DD2()
 {
  ...
  char tmp[200];  int distav_1; char str[300];
  ...
  else if (CntAVBefore == 1)
    {
      ...
      tmp[8]=0;
        distav_1 = Reverse_Date(tmp);
           ...
           sprintf(str,"COMPD1D2 BEFORE=1  AVANC_1 %s dist=%d aa=%d mm=%d jj=%d",tmp,dist,an_1,mois_1,jour_1);
           Application->MessageBox(str,"ecopl",MB_OK);
        ...
        if (strcmp(tmp,DATEAVANC)==0)
          {
           ...
           sprintf(str,"COMPD1D2 AVANC_1 %s dist=%d aa=%d mm=%d jj=%d",tmp,dist,an_1,mois_1,jour_1);
           Application->MessageBox(str,"ecopl",MB_OK);
           Avant = cbAvantX->Text;
           if (Avant == "1 Semaine Avant") DD1=dist - 7;
           else if ( Avant == "2 Semaines Avant") DD1=dist - 14;
      ...
      int ARow, AnsiString &Value, bool &Valid)
{
 ...
 // Modifify asAvanc  before
    if (ch==29) asAvanc->Cells[18][row_avancement]=AnsiString(tmp);
    if (ch==28) asAvanc->Cells[19][row_avancement]=AnsiString(tmp);
    if (ch==57) asAvanc->Cells[5][row_avancement]=AnsiString(tmp);
...
void __fastcall TplStandard::btRAZClick(TObject *Sender)
{
 ...
 if (nb==0) {Application->MessageBoxA("Pas de fichier actualisation trouvé",m_ecoplan,MB_OK);
             return; }

 // fichier actualisation in lastfile.

 strcpy(nameavanc,DatabaseName); strcat(nameavanc,"-");
 strcat(nameavanc,lastsuffix);
...
void __fastcall TplStandard::Modif_All_Links()
{
 ...
 char dfinpred[20],ddeb[20];  int cal;
 char tmp[50];
 // colonne 0 a vide
...
void __fastcall TplStandard::Cout_Ressources()
{
  if (rbCout->Checked)
   {
    Label77->Caption = "Saisie des Coûts des Tâches";
    Label143->Caption = "Echéancier des Couts de la Sélection";
    Button7->Caption = "Sauvegarde des Coûts";
    avc->Cells[6][0]="Coût +";
    avc->Cells[7][0]="Coût -";
