...
void __fastcall TplStandard::Refresh_Concern(bool strict)
{
  ...
  char msg[5000]; char DATEAVANC_REVERSED[500];
  int dist_avanc_1;  int nbavanc;
  int dist_xx;  int xx;
  ...
  //sprintf(msg,"DATEAVANC %s  AVANC=%s",DATEAVANC,AVANC);
  //Application->MessageBox(msg,"Ecopl",MB_OK);

  // dist1=Convert_Date(AnsiString(DATEAVANC));


  //sprintf(msg,"nb avanc = %d DATEAVANC %s",nbavanc,DATEAVANC);
  //Application->MessageBox(msg,"Ecopl",MB_OK);
  ...
  if (nbavanc==1 || nbavanc==0)
    {  strcpy(adv,asAvanc->Cells[8][i].c_str()); strcpy(dt1,adv);   // debut prev
       ...
       sprintf(msg,"NBAVANC=0 ou 1  AA= %d MM = %d JJ=%d",a1,m1,j1);
            Application->MessageBox(msg,"Ecopl",MB_OK);
        ...
        if (dist_xx==distavanc)
           {
            ...
            sprintf(msg,"Valeur de I %d PARAM[xx] = %s   IDOK  AA= %d MM = %d JJ=%d",i,params[xx],a1,m1,j1);
            Application->MessageBox(msg,"Ecopl",MB_OK);

            sprintf(msg,"IDAVANC-1 XX=%d   *** DATEAVANC %s distavanc=  %d  dist_avanc_1 %d",xx,DATEAVANC,distavanc,dist_avanc_1);
            Application->MessageBox(msg,"Ecopl",MB_OK);

            dist_avanc_1== distavanc-15; // convert_date(a1+2000,m1,j1);
  ...
  sprintf(msg,"DATEAVANC %s distavanc %d  distavanc-1 %d",DATEAVANC,distavanc,dist_avanc_1);
  Application->MessageBox(msg,"Ecopl",MB_OK);
  ...
  for (i=1;i<cntavanc;i++)
    {
      ...
      // sprintf(tmp,"AVANC=%d DEBPREV= %d  cntavanc=%d",distavanc,distdebprev,i);
