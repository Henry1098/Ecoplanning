...
void __fastcall TPrepPlanning::SaveData(int mode)
{
 ...
 AnsiString abc="Planning executé";
 strcpy(TempKey,abc.c_str());}
...
change = false;
if (mode !=0) Application->MessageBoxA("Planning Sauvegardé",m_ecoplan,MB_OK);
}
...
void __fastcall TPrepPlanning::Button6Click(TObject *Sender)
{
 ...
 char str_ = ae[1];
    if(isdigit(str_))
    {
     ShowMessage("Veuillez changer de nom! Pour enregistrer un Planning, il faut que la désignation commence obligatoirement par une lettre !!!");
     return;
    }
  }
  if(ae=="")
...
void __fastcall TPrepPlanning::Button9Click(TObject *Sender)
{
 ...
 Imp->Caption = "Import Données Planning";
 Imp->ShowModal();
 delete Imp;
*/

 //Ghost->New_Planning();
 //Application->MessageBoxA("Planning Initialisé avec les Valeurs par Défaut",m_ecoplan,MB_OK);
}
//---------------------------------------------------------------------------
...
if (change) { SaveData(1); change=0; }
 ...
 AnsiString ae= Label3->Caption;
 AnsiString ae2= Label5->Caption;

 AnsiString cc=direct+"\\Ref2\\"+ae+".txt";
 AnsiString cc2=direct+"\\Ref2\\"+ae2+".txt";
...
void __fastcall TPrepPlanning::btAppliquerClick(TObject *Sender)
{
 ...
 if (Label7->Caption == "" && P_GRILLE[pc] == 'O')
   {
    Application->MessageBoxA("Sélectionner une Grille",m_ecoplan,MB_OK);
    return;
   }
 ...
 if (cbEbauche->Checked == true)
   {
    ...
    if (Application->MessageBoxA("Attention : Si Oui, Vous Revenez a l'ébauche initiale",m_ecoplan,MB_YESNO) == IDNO)
      {
