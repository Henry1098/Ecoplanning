...
uses
  ...
  ShellApi, PictureContainer, AsgCheck, AsgHTMLE, BaseGrid, Mask
...
const
  ...
  BLD_VER = 0; // Build nr.
  DATE_VER = 'Nov, 2004'; // Month version
...
type
  ...
  TGridExportState = (esExportStart, esExportNewRow, esExportDone);
  ...
  published
    ...
    property CommentColor: TColor read FCommentColor write SetCommentColor;
  ...
  published
    ...
    property SelectOnRigthClick: Boolean read FSelectOnRightClick write FSelectOnRightClick default False;
  ...
  published
    ...
    property Show: Boolean read FSortShow write SetShow;
  ...
  protected
    ...
    procedure AdvanceEdit(ACol,ARow: Integer;Advance,Show,Frwrd,Recurs: Boolean);
...
//    procedure AddCheckBoxEx(ACol,ARow: Integer;State,Data: Boolean;HAlign: TCellHalign;VAlign: TCellVAlign);
  ...
  published
    ...
    property ActiveCellColorTo: TColor read FActiveCellColorTo write SetActiveCellColorTo;    
...
begin
  ...
  begin
    ...
    if Key = VK_RETURN then
      FGrid.AdvanceEdit(FGrid.Col,FGrid.Row,False,True,True,False);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        FGrid.FOnComboChange(FGrid,FGrid.Col,FGrid.Row,ItemIndex,Items[ItemIndex]);
      end;

      if Assigned(FGrid.FOnComboObjectChange) then
      begin
        FGrid.FOnComboObjectChange(FGrid,FGrid.Col,FGrid.Row,ItemIndex,Items[ItemIndex],Items.Objects[ItemIndex]);
      end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            AdvanceEdit(Col,Row,True,True,True,False);
    ...
    begin
      ...
      begin
        ...
        begin
          am := Navigation.AdvanceDirection;
          Navigation.AdvanceDirection:=adLeftRight;
          AdvanceEdit(Col,Row,True,True,False,False);
          Navigation.AdvanceDirection:=am;
          Key := VK_RETURN;
        end;
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (NRow <> -1) then
      begin
        SetNodeSpan(NRow,GetNodeSpan(NRow) + 1);
...
begin
  BC := BaseCell(ACol,ARow);
  Result := (CellTypes[BC.X,BC.Y] in [ctCheckBox,ctDataCheckBox,ctVirtCheckBox,ctRadio,ctButton]);
end;
...
begin
  ...
  if nc > ColCount - VisibleColCount + FixedCols  then
    nc := ColCount - VisibleColCount + FixedCols;

  LeftCol := nc;

  if nr > RowCount - VisibleRowCount + FixedRows then
    nr := RowCount - VisibleRowCount + FixedRows;
...
begin
  ...
  begin
    ...
    begin
      ...
      {$ENDIF}
      {$IFDEF TMSDOTNET}
        UpdateEditingCell(ACol,ARow,DateToStr(EditDate.DateTime))
      {$ENDIF}
      else
  ...
  begin
    ...
    begin
      ...
      {$ENDIF}
      {$IFDEF TMSDOTNET}
      UpdateEditingCell(ACol,ARow,TimeToStr(EditDate.DateTime));
      {$ENDIF}
...
begin
  ...
  begin
    ...
    FEditDisable := False;
    if not (goEditing in Options) and CanEdit then
    begin
       FEditChange := True;
       Options := Options + [goEditing];
    end;
...
begin
  ...
  begin
    ...
    if (s <> '') then
    begin
      if not (cg.CellHAlign in [haBeforeText,haAfterText]) then
        w := 0;
...
procedure TAdvStringGrid.AdvanceEdit(ACol,ARow: Integer;advance,show,frwrd,recurs: Boolean);
var
...
//  ARow := Span.Y;
 ...
 //Handle backward case
  ...
  if not IsEditable(rm,ARow) then
    AdvanceEdit(ACol,ARow,Advance,Show,Frwrd,True)
  else
  begin
    CanChange := True;
...
begin
  ...
  begin
    AdvanceEdit(Col,Row,False,False,True,False);
...
begin
  ...
  GetVisualProperties(RemapColInv(ACol),ARow,AState,False,True,True,Canvas.Brush,AColorTo,Canvas.Font,HAlign,VAlign,WW);
...
var
  ...
  begin
    ...
    begin
      ...
      {$IFDEF TMSDOTNET}
      DrawProgressPie(Canvas,Rect(TgtRect.left,TgtRect.Top,TgtRect.left+20,TgtRect.Top+20),CellGraphic.CellColor,CellGraphic.CellAngle);
      {$ENDIF}
    end;
    ...
    begin
      ...
      begin
        ...
        {$IFDEF TMSDOTNET}
        if CellGraphic.CellBoolean then
          DrawProgressLin(Canvas,r,CellGraphic.CellColor, CellGraphic.CellTextFGColor,
            CellGraphic.CellBKColor,CellGraphic.CellTextBKColor,Ints[ACol,ARow],ControlLook.ProgressMarginX,ControlLook.ProgressMarginY)
        else
          DrawProgressLin(Canvas,r,CellGraphic.CellColor,CellGraphic.CellColor xor $FFFFFF,
            CellGraphic.CellBKColor,CellGraphic.CellBKColor xor $FFFFFF,Ints[ACol,ARow],ControlLook.ProgressMarginX,ControlLook.ProgressMarginY);
        {$ENDIF}
    ...
    begin
      ...
      begin
        ...
        begin
          Canvas.MoveTo(r.Left + CellNode.NodeIndent div 2,r.Top);
          Canvas.LineTo(r.Left + CellNode.NodeIndent div 2,r.Top + 6);
        end;
  ...
  begin
    ...
    if WordWrap and not MultiLineCells and FEnhTextSize then
      DrawStyle := DrawStyle or DT_END_ELLIPSIS;
...
begin
  ...
  begin
    if FActiveCellShow and
      (((Row = ARow) and (OCol = 0) and (FixedCols > 0)) or
      ((Col = OCol) and (ARow = 0) and (FixedRows > 0))) then
       DrawGradient(Canvas, ActiveCellColor,ActiveCellColorTo,10,ARect,False)
    else
...
begin
  ...
  begin
    ...
    if Assigned(UndoRedo) then
      UndoRedo.RegisterChange(ROldCol,ROldRow,OriginalCellValue,Value);
  ...
  begin
    ...
    begin
      if (Col >= LeftCol) and (Col < LeftCol + VisibleColCount) and
         (Row >= TopRow) and (Row < TopRow + VisibleRowCount) then
         begin
           ShowInplaceEdit;
           Msg.Result := 0;
           Exit;
...
begin
  ...
  begin
    ...
    begin
      ...
      if CanEdit then
      begin
        if HasCheckBox(RCol,Row) and (Msg.CharCode = VK_SPACE) then
        begin
        ...
        if IsRadio(RCol,Row) and (Msg.CharCode = VK_SPACE) then
        begin
...
begin
  ...
  if MouseActions.SelectOnRigthClick then
  begin
    if (x <> -1) and (y <> -1) then
...
begin
  ...
  begin
    ...
    if (x = 0) and (y >= FixedRows) and (FNumNodes > 0) and not MouseActions.RowSelect then
      Exit;
...
begin
  ...
  repeat
    ...
    begin
      if SortDir = Compare(Col,Idx,Idx + 1,SortSettings.Direction) then
      begin
...
begin
  ...
  if not (csDestroying in ComponentState) then
    CellsChanged(Rect(0,RowIndex,ColCount - 1,RowIndex + RowCount));
end;
...
var
  i,j,mc,mr: Integer;
  s,al,ac,av,afs,afe,afc,aff,ass,ase,tablestyle: string;
...
begin
  ...
  if MouseActions.RowSelect then
    i := PasteFunc(0,Selection.Top)
  else
...
begin
  ...
  try
    //FExcel := Activator.CreateInstance( System.Type.GetTypeFromProgID('excel.application' ) );
    FExcel := CreateOleObject('excel.application');
  except
    Screen.cursor := crDefault;
    raise EAdvGridError.Create('Excel OLE server not found');
...
begin
  ...
  begin
    ...
    if not ((IsNode(di) or IsSummary(di)) and (GroupColumn <> -1)) and IsBaseCell(ACol, di) then
    begin
      try
        if doh then
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if Assigned(FOnAnchorHint) then
          FOnAnchorHint(self,Row,RCol,Anchor);
        HintStr := Anchor;
      end;
    end;
    ...
    csize := GetCellTextSize(RCol,Row,False);
...
begin
  ...
  begin
    for j := FixedCols to AllColCount do
      ClearPropCell(j,i);
  end
  else
  begin
...
//   (Message.Msg = CM_INVALIDATE) or
  ...
  begin
    ...
    begin
      ...
      if FControlList.HasHandle(message.lParam) then
        SendMessage(message.lParam,WM_COMMAND,message.wparam,message.lparam);
    end;
  end;
...
begin
  ...
  SetFocus;
  if Key = VK_RETURN then
    AdvanceEdit(Col,Row,False,True,True,False);
...
begin
  ...
  if Allow then
    ...
    begin
      ...
      if Key = VK_RETURN then
      begin
        AdvanceEdit(Col,Row,False,True,not (ssShift in Shift),False);
        Key := 0;
        Exit;
      end;
  ...
  begin
    ...
    begin
      if Navigation.AdvanceAuto then
        AdvanceEdit(Col,Row,true,false, true, false)
      else
