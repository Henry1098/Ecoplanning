...
// #include "ikey.h"      Version sans IKEY
      ...
      TCloseAction &Action)
{
 ...
 // ledState = IKEY_PROP_LED_ON;
 // status = ikey_SetProperty( handle,ledState,NULL, NULL,0);
 // µµµµµ ikey_CloseDevice( handle );
 // µµµµµ ikey_DeleteContext( handle );
 Sleep(1000);
 Action = caFree;
}
//---------------------------------------------------------------------------
...
void __fastcall TplStandard::Prepare_Draw_Planning()  // main routine  lance tri et filtre
{
 ...
 //  if (pc==3) Remplir_asTri_Avanc();
  ...
  if (pc<4 && P_DETAIL[pc]==1)
    {
     if (P_LIENS[pc][0]=="Oui" || P_LIENS[pc][1]=="Oui") Trace_Liens();    // 1 pour previsionnel critique
    }
...
void __fastcall TplStandard::Trace_Liens()
{
 int indx, count, nbsc;
 char tmp[500];  char marg[20];
 int x1,y1,x2,y2,y11,y22; int rc,mrg;
          ...
          if (rc > 0)
           {
            ...
            else if (mrg > 0 && P_LIENS[pc][1]=="Oui")
                {
...
int __fastcall   TplStandard::InitKey()
{
 int ct,i,j;
...
int __fastcall TplStandard::check_key(int val)
{
 /*
 // int playcode;
 request=val;
...
int __fastcall TplStandard::DecCounter()
{
  char tmp[MAX_PATH];
...
bool __fastcall TplStandard::Check_Config()
{
 ...
 if (fp==NULL)   // new installation
    {
     ...
     fputs(LK,fp);
     fclose (fp);
     // Application->MessageBoxA(M[545].c_str(),"Ecoplanning",MB_OK); // you got a temporary licence for ....
     REGIST = false;
...
// l=strlen(LK);
 ...
 if (strcmp(MAC_ADDR,tmp) != 0)
    {
     Application->MessageBox("Clé de la licence incorrecte","Ecoplanning",MB_OK); // "Licence key doesn't match your Client Code"
     return false;
    }
 ...
 if (strlen(EXPDATE) != 10)
     {
      Application->MessageBox("Date de licence incorrecte","AVSA",MB_OK); // "Invalid date format - Contact your distributor"
      return false;
     }
  strcpy(tmp,EXPDATE);
  p=tmp;
  ...
  if (year1 < 2010) { Application->MessageBox("Année date licence incorrecte","Ecoplanning",MB_OK); return false ; }    // "Invalid year"
  if (month1 < 1 || month1 >13) { Application->MessageBox("Mois date licence incorrect","Ecoplanning",MB_OK); return false ;  }   // "Invalid Month"
  if (day1< 1 || day1 >31){ Application->MessageBox("Jour date licence incorrect","Ecoplanning",MB_OK); return false;  }   // "Invalid Day"

   dist1 =  (year1-2000)*365 + month1*31+day1;
   if (dist > dist1)  { Application->MessageBox("Votre licence a expiré","Ecoplanning",MB_OK);return false; }     // "Licence has expired - Please contact your distributor"
   if (dist > dist1-5)
        {
          delta = dist-dist1-5;
          strcpy(mess,"Votre licence va expirer le ");strcat(mess,EXPDATE);    // "Warning : you licence will expire the "
...
void __fastcall TplStandard::Enum_MAC()
{
  int nb,retc;
  ...
  for (int i = 0; i < AdapterList.length ; ++i)
  {
    if (GetAdapterInfo(AdapterList.lana[i], mac_addr))
    {
      ...
      if (i==0)  {  LabMAC->Caption = mac_addr;   strcpy(MAC_ADDR,mac_addr.c_str()); }
...
bool __fastcall TplStandard::GetAdapterInfo(int adapter_num, AnsiString &mac_addr)
{
